...
const table = 'letter_out_handled_by_user'
...
getByLetterOutId = (req,res) => {
    ...
    db.query(`SELECT a.*,b.full_name FROM ${table}  a LEFT JOIN ${tb_employees} b ON a.employee_id = b._id WHERE a.letter_out_id = ? ` , id ,(err, rows, field) => {
        if (err){
            console.log(err)
            response.error(rows,err.sqlMessage,res)
        } else{
...
create = (req,res) => {
    ...
    const body = req.body
    deleteLetterOuthandled(req,res)
    for (var i = 0; i < body.handled_by_user_id.length; i++) {
        bd = {
            letter_out_id : body._id,
            employee_id : body.handled_by_user_id[i].value
        }
