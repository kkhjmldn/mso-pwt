...
Find = (req,res) => {
    ...
    db.query(`SELECT a.* FROM ${table}  a WHERE a.method like '%${key}%' OR a.description like '%${key}%' `  ,(err, rows, field) => {
        if (err){
            console.log(err)
            response.error(rows,err.sqlMessage,res)
        } else{
...
create = (req,res) => {
    ...
    var _id = "CLT" + dateformat(new Date(), "yyyymmddhMMss");
    //NEXT 
