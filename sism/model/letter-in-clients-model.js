...
getByletterinId = (req,res) => {
    ...
    db.query(`SELECT a.*,b.client_code, b.client_name FROM ${table}  a LEFT JOIN ${tb_clients} b ON a.client_id = b._id WHERE a.letter_in_id = ? ` , id ,(err, rows, field) => {
        if (err){
            console.log(err)
            response.error(rows,err.sqlMessage,res)
        } else{
