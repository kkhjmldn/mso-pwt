...
getAll = (req, res) => {
    ...
    if (key !=='' && key !== undefined) {
        search+=` WHERE  (a.priority  like '%${key}%' OR  a.max_respond_time like '%${key}%'  )  `
    }
   try{
...
create = (req, res) => {
    const body = req.body
    body._id = 'PRI' + dateformat(new Date(), 'yyyymmddHHMMss') + body.created_by.substr(body.created_by.length - 4, body.created_by.length)
